<?php
/**
 * NEW MouseFight API — Get one full Genesis mouse fighter profile for one Discord user.
 *
 * Plain language for DEVS FOR DECADES:
 * This endpoint is read-only. It returns one named Genesis mouse fighter profile
 * owned by a Discord user so the Discord bot can snapshot joins/challenges.
 *
...
try {
    ...
    $tokenIdInput = trim((string)($_GET['token_id'] ?? ''));
    if ($tokenIdInput === '') {
        mousefight_json_response([
            'success' => false,
    ...
    /**
     * Load exactly one named Genesis mouse for this Discord user and token.
     *
     * Plain language for DEVS:
     * tbl_nft_ownership can contain historical rows. The current_ownership CTE
     * keeps MouseFight from snapshotting an outdated owner row.
     */
    $namedMiceStmt = $db->prepare("
        ...
        LIMIT 1
    ");
    $namedMiceStmt->execute([$discordId, $tokenIdInput]);
    $mouse = $namedMiceStmt->fetch(PDO::FETCH_ASSOC);

    if (!$mouse) {
        mousefight_json_response([
            'success' => false,
            'error' => 'No eligible named Genesis mouse found for this user and token_id.',
            'data' => [
    ...
    $traitStmt = $db->prepare("
        ...
        WHERE COALESCE(collection, 'genesis') = 'genesis'
          AND token_id = ?
    ");
    $traitStmt->execute([$tokenIdInput]);
    ...
    $abilityStmt = $db->prepare("
        ...
        WHERE COALESCE(collection, 'genesis') = 'genesis'
          AND token_id = ?
    ");
    $abilityStmt->execute([$tokenIdInput]);
    ...
    if (!$canFight) {
        mousefight_json_response([
            'success' => false,
            'error' => 'Selected mouse does not meet MouseFight eligibility rules.',
            'fighter' => $profile,
            'criteria' => [
    ...
    mousefight_json_response([
        ...
        'snapshot_rule' => 'Store this response in the MouseFight runtime/DB participant snapshot when a fighter joins.',
    ]);
