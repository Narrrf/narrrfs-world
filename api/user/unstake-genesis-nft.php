 ...
 *   contain the exact unfreeze challenge message.
...
/**
 * Validate Genesis Mouse Freezer unfreeze activation proof.
 *
 * Plain language for DEVS:
 ...
 * contain the exact unfreeze challenge message.
 */
function validate_unfreeze_activation_proof(array $requestData, array $challenge): string
{
    ...
    if ($memoSignature === '') {
        json_response([
            'success' => false,
            'error' => 'Solana Memo transaction signature is required for production unfreeze activation.',
...
try {
    ...
    $memoSignature = validate_unfreeze_activation_proof($requestData, $challenge);
