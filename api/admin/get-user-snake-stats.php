...
try {
    ...
    $response = [
        ...
        'statistics' => [
            'total_games' => (int)$snakeStats['total_games'],
            'best_score' => (int)$snakeStats['best_score'],
            'average_score' => round((float)$snakeStats['average_score'], 2),
            'total_score' => (int)$snakeStats['total_score'],
            'last_game' => $snakeStats['last_game'],
            'first_game' => $snakeStats['first_game']
        ],
        'recent_games' => $recentGames,
        'seasonal_performance' => $seasonalPerformance,
        'status' => $snakeStats['total_games'] > 0 ? 'Active' : 'Inactive'
    ];

    echo json_encode($response);

} catch (Exception $e) {
    error_log('Error in get-user-snake-stats: ' . $e->getMessage());
