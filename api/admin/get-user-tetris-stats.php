...
try {
    ...
    $result = $stmt->execute();
    $tetrisStats = $result->fetchArray(SQLITE3_ASSOC);

    // Get user's recent Tetris games
    $stmt = $db->prepare("SELECT 
        score,
        timestamp,
        season,
    ...
    $response = [
        ...
        'statistics' => [
            'total_games' => (int)$tetrisStats['total_games'],
            'best_score' => (int)$tetrisStats['best_score'],
            'average_score' => round((float)$tetrisStats['average_score'], 2),
            'total_score' => (int)$tetrisStats['total_score'],
            'last_game' => $tetrisStats['last_game'],
            'first_game' => $tetrisStats['first_game']
        ],
        'recent_games' => $recentGames,
        'seasonal_performance' => $seasonalPerformance,
        'status' => $tetrisStats['total_games'] > 0 ? 'Active' : 'Inactive'
    ];

    echo json_encode($response);

} catch (Exception $e) {
    error_log('Error in get-user-tetris-stats: ' . $e->getMessage());
