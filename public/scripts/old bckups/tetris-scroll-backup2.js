...
// 🎮 Keyboard controls (WASD + Arrow keys) – Desktop
document.addEventListener("keydown", e => {
  const moveLeft  = () => !collide(current.shape, current.row, current.col - 1) && current.col--;
  const moveRight = () => !collide(current.shape, current.row, current.col + 1) && current.col++;
  const moveDown  = () => drop();
  const rotate    = () => rotatePiece();
...
// 🚫 Prevent default scroll on key press (arrows + space)
...
// 📱 Touch controls – Mobile Swipe (scroll-safe)
let touchStartX = 0;
let touchStartY = 0;
